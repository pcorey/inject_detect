...
class UnexpectedQuery extends React.Component {
    ...
    markAsExpected = () => {
        ...
        this.props
            .markAsExpected(unexpectedQuery.application.id, unexpectedQuery.id)
            .catch(err => {
                console.error(err);
            })
            .then(() => {
    ...
    markAsHandled = () => {
        ...
        this.props
            .markAsHandled(unexpectedQuery.application.id, unexpectedQuery.id)
            .catch(err => {
                console.error(err);
            })
            .then(() => {
    ...
    render() {
        ...
        if (redirect) {
            return <Redirect to={`/application/${unexpectedQuery.application.id}`} />;
        }
                ...
                <div className="ij-unexpected-query ui mobile stackable grid">
                    ...
                    <div className="six wide column">
                        ...
                        {!unexpectedQuery.handled
                            ? <button
                                  disabled={unexpectedQuery.expected || unexpectedQuery.handled}
                                  className={`ui labeled icon button ${markingAsExpected ? 'loading' : ''}`}
                                  onClick={this.markAsExpected}
                              >
                        ...
                        {!unexpectedQuery.expected
                            ? <button
                                  disabled={unexpectedQuery.expected || unexpectedQuery.handled}
                                  className={`ui brand labeled icon button ${markingAsHandled ? 'loading' : ''}`}
                                  onClick={this.markAsHandled}
                              >
                    ...
                    <div className="ten wide column">
                        ...
                        <p className="instructions">
                            This query has been detected
                            {' '}
                            <strong>{unexpectedQuery.seen}</strong>
                            {' '}
                            times in the "
                            {unexpectedQuery.application.name}
                            " application. It was last seen
                            {' '}
                            <strong><Moment fromNow>{unexpectedQuery.queriedAt}</Moment></strong>
                            .
                        </p>
                        <p className="instructions">
                            If this doesn't look like a type of query your application would make, it may be the result of a
                            {' '}
                            <a href="http://www.injectdetect.com/education/what-is-nosql-injection/">
                                NoSQL Injection attack
                            </a>
                            . Try to find locations in your application making similar queries and look for potential injection vulnerabilities.
                        </p>
                        {JSON.parse(unexpectedQuery.similarQuery)
                            ? <div>
                                  <p className="instructions">
                                      Here's the most
                                      {' '}
                                      <a href="http://www.injectdetect.com/education/how-do-we-detect-nosql-injection/#similar-expected-queries">
                                          similar expected query
                                      </a>
                                      {' '}
                                      we found when we spotted this query:
                                  </p>
                            ...
                            : <p className="instructions">
                                  ...
                                  {' '}
                                  when we spotted this query.
                              </p>}
                        <p className="instructions">
                            Once you've identified where this unexpected query came from and fixed the vulnerability, mark this query as
                            {' '}
                            <strong>handled</strong>
                            .
                        </p>
                        <p className="instructions">
                            Otherwise, if this is a query that your application is expected to make, mark it as
                            {' '}
...
`,
    {
        props: ({ mutate }) => ({
            markAsExpected: (application_id, query_id) =>
                mutate({
...
`,
    {
        props: ({ mutate }) => ({
            markAsHandled: (application_id, query_id) =>
                mutate({
                    variables: { application_id, query_id },
                    refetchQueries: [
                        {
